 ...
 * Centralized state management extracted from scan processor with minimal adaptations
 * 
...
/**
 * Grading Hub DataStore
 * Extends BaseDataStore with grading-specific state management
 */
class GradingDataStore extends BaseDataStore {
    constructor() {
        super();
        
        // Add grading-specific state properties
        this._gradingState = {
            ...
            gradingResults: [],
            studentAnswers: [],
            currentPage: 'rubric-setup' // 'rubric-setup' | 'grading-interface'
        };
    }

    // Grading-specific getters/setters
    getSelectedProfile() {
        return this._gradingState.selectedProfile;
    }
    
    setSelectedProfile(profile) {
        this._gradingState.selectedProfile = profile;
        eventBus.emit('grading:profile-changed', { profile });
    }

    getAvailableProfiles() {
        return this._gradingState.availableProfiles;
    }
    
    setAvailableProfiles(profiles) {
        this._gradingState.availableProfiles = profiles;
        eventBus.emit('grading:profiles-loaded', { profiles });
    }

    getCurrentRubric() {
        return this._gradingState.currentRubric;
    }
    
    setCurrentRubric(rubric) {
        this._gradingState.currentRubric = rubric;
        eventBus.emit('grading:rubric-changed', { rubric });
    }

    getGradingResults() {
        return this._gradingState.gradingResults;
    }
    
    setGradingResults(results) {
        this._gradingState.gradingResults = results;
        eventBus.emit('grading:results-changed', { results });
    }

    getStudentAnswers() {
        return this._gradingState.studentAnswers;
    }
    
    setStudentAnswers(studentAnswers) {
        this._gradingState.studentAnswers = studentAnswers;
        eventBus.emit('grading:student-answers-changed', { studentAnswers });
    }

    getCurrentPage() {
        return this._gradingState.currentPage;
    }
    
    setCurrentPage(page) {
        this._gradingState.currentPage = page;
        eventBus.emit('grading:page-changed', { page });
    }

    // Grading-specific validation helpers
    hasSelectedProfile() {
        return this._gradingState.selectedProfile !== null;
    }

    hasCurrentRubric() {
        return this._gradingState.currentRubric !== null;
    }

    hasGradingResults() {
        return this._gradingState.gradingResults.length > 0;
    }

    canStartGrading() {
        return this.hasSelectedProfile() && this.hasCurrentRubric();
    }

    // Reset grading-specific state
    resetGradingState() {
        this._gradingState = {
            selectedProfile: null,
            availableProfiles: [],
            currentRubric: null,
            gradingResults: [],
            currentPage: 'rubric-setup'
        };
        eventBus.emit('grading:state-reset');
        console.log('📊 GradingDataStore: Grading state reset');
    }
}

// Export singleton instance for scan processor (original behavior)
export default new BaseDataStore();

// Export grading hub singleton
export const gradingDataStore = new GradingDataStore();

// Export base class for further extension
