/**
 * File Management Service - Wizard Integration
 * Adapted from ScanProcessor for grading wizard workflow
 * Uses gradingDataStore and wizard-specific DOM elements
 */
...
export default class FileManagement {
    ...
    /**
     * Setup file upload functionality for wizard step
     */
    setupFileUpload() {
        ...
        if (!fileInput || !uploadZone || !browseBtn) {
            console.warn('FileManagement: Required DOM elements not found');
            return;
        }
        ...
        fileInput.addEventListener('change', (e) => {
            if (e.target.files.length > 0) {
                this.handleFileSelect(e.target.files[0], 'rubric');
            }
        });
        ...
        uploadZone.addEventListener('drop', (e) => {
            ...
            if (e.dataTransfer.files.length > 0) {
                this.handleFileSelect(e.dataTransfer.files[0], 'rubric');
            }
        });
        ...
        // PDF page selection functionality
        this.setupPageSelection();
    }

    /**
     * Setup PDF page selection functionality
     */
    setupPageSelection() {
        const pageSelectionType = document.getElementById('page-selection-type');
        ...
        console.log('🔧 FileManagement: Setting up page selection. Elements found:', {
            pageSelectionType: !!pageSelectionType,
            customRangeInput: !!customRangeInput,
            customPagesInput: !!customPagesInput,
            previewBtn: !!previewBtn
        });

        if (pageSelectionType && customRangeInput) {
            pageSelectionType.addEventListener('change', () => {
                console.log('🔧 Page selection changed to:', pageSelectionType.value);
                if (pageSelectionType.value === 'custom') {
            ...
            console.warn('🔧 FileManagement: Missing required elements for page selection');
        }

        if (customPagesInput) {
            customPagesInput.addEventListener('input', () => {
                this.validatePageRange(customPagesInput.value);
            });
        }
        ...
        if (previewBtn) {
            previewBtn.addEventListener('click', () => {
                if (this.selectedFile) {
                    // Emit event to wizard to show PDF preview
                    eventBus.emit('pdf:show-preview', { file: this.selectedFile });
                }
            });
        }
    }
    ...
    removeFile() {
        ...
        // Emit event to update wizard navigation
        eventBus.emit('file:uploaded', { file: null, uploadType: null });
    }
